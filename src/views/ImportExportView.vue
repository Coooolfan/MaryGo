...
import { GoMatchesSlim } from '@/assets/utils/GoMatch'
import type { GoMatchSlim } from '@/assets/utils/GoMatch'
...
const selectedMatch = ref<GoMatchSlim | null>(null)

function handleMatchSelect(match: GoMatchSlim) {
  console.log(match)
...
<template>
  ...
  <nut-popup v-model:visible="showFormatSelector" position="bottom">
    <nut-picker v-model="formateSelectorValue" :columns="columns" title="选择文件格式" @cancel="showFormatSelector = false"
      @confirm="handleFormateSelector" three-dimensional />
  </nut-popup>
  <nut-popup :visible="showMatchSelector" position="bottom" closeable round
    :style="{ maxHeight: '80%', padding: '1.25rem 0.5rem 1rem 0.5rem' }" @click-close-icon="showMatchSelector = false"
    @click-overlay="showMatchSelector = false">
    <p class="text-lg font-bold mb-4 ml-4">选择棋谱</p>
    <div class="flex flex-col gap-3">
      <GoMatchCardSlim v-for="match in GoMatchesSlim" :key="match.id" :match="match"
